...
COMPOSE_FILE="$DOCKER_FOLDER/compose/$HOSTNAME/9router.yml"
...
if [[ -n "$JWT_SECRET" ]]; then
	sudo sed -i "s|JWT-SECRET-PLACEHOLDER|$JWT_SECRET|" "$COMPOSE_FILE"
...
if [[ -n "$INITIAL_PASSWORD" ]]; then
	sudo sed -i "s|INITIAL-PASSWORD-PLACEHOLDER|$INITIAL_PASSWORD|" "$COMPOSE_FILE"
...
sudo sed -i "s|API-KEY-SECRET-PLACEHOLDER|$API_KEY_SECRET|" "$COMPOSE_FILE"
sudo sed -i "s|MACHINE-ID-SALT-PLACEHOLDER|$MACHINE_ID_SALT|" "$COMPOSE_FILE"
