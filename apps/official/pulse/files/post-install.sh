...
PULSE_TOKEN=$(sudo docker exec "$APP_SNAME" cat /data/.bootstrap_token 2>/dev/null)
...
if [[ -n "$PULSE_TOKEN" ]]; then
    ...
    echo "You can retrieve it manually with: sudo docker exec $APP_SNAME cat /data/.bootstrap_token"
fi
